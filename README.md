...
Add `vrpc-hpp` to your `CMakeLists.txt` using `FetchContent`. We safely bundle the necessary Boost dependencies by default (but will automatically use your system's Boost if you have one installed to prevent conflicts).
...
# Link your target against vrpc::vrpc
# (This automatically configures include paths, Boost, and threading!)
...
### 4. Build and Start the Agent (Secure by Default)

VRPC is designed for the modern web and uses TLS/SSL to connect securely to your broker (`mqtts://` or `ssl://`). This requires OpenSSL.

**1. Install OpenSSL:**

- **Linux (Ubuntu/Debian):** `sudo apt-get install libssl-dev`
- **Linux (Alpine):** `apk add openssl-dev`
- **Windows (via vcpkg):** `vcpkg install openssl:x64-windows`

**2. Configure and Build:**
Tell CMake to enable TLS. (If on Windows, make sure to pass your vcpkg toolchain file so CMake finds OpenSSL).

```bash
mkdir build && cd build
cmake .. -DVRPC_WITH_TLS=ON
make

# Start the agent and connect securely!
./my_vrpc_agent -d my_domain -a cpp_edge_device -b mqtts://broker.hivemq.com:8883
```

_(Note: You can also force VRPC to use its bundled Boost headers by passing `-DVRPC_USE_BUNDLED_BOOST=ON` during configuration)._

### 5. Call it from Anywhere (e.g., Node.js / React)

Once your C++ agent is running, you can interact with it transparently from any VRPC client. Let's create a quick Node.js client!

**1. Set up a new Node.js project:**
Create a new directory and install the VRPC Javascript package.

```bash
mkdir vrpc-client && cd vrpc-client
npm install vrpc
```

**2. Create the client code (`index.js`):**
